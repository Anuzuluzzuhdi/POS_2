...
class KategoriController extends Controller
{
        ...
        return DataTables::of($kategori)
            ...
            ->addColumn('aksi', function ($kategori) {
                ...
                $btn .= '<form class="d-inline-block" method="POST" action="' . url('/kategori/' . $kategori->kategori_id) . '">'
                    . csrf_field() . method_field('DELETE') . '<button type="submit" class="btn btn-danger btn-sm" onclick="return confirm(\'Apakah anda yakin 
                menghapus data ini?\');">Hapus</button></form>';
                return $btn;
            })
            ->rawColumns(['aksi'])
            ->make(true);
    ...
    public function destroy(string $id)
    {
        ...
        try {
            ...
            return redirect('/kategori')->with('error', 'Data kategori tidak bisa dihapus karena terdapat data yang 
                        terkait');
        }
    }
}
